...
A plug-in for markdown-vault-template-shaped vaults — overlays retrieval, semantic search, and citation chains onto a vault that runs standalone without it. Works with any markdown vault that uses wikilinks; the vault remains the source of truth and the engine is best-effort enrichment.
...
```bash
uv run vault-engine mcp --vault ~/Projects/markdown-vault
```
...
```bash
uv run vault-engine hook install --vault ~/Projects/markdown-vault
```
